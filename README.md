...
- Live Polymarket odds and result-locking only cover the 52 group matches that were in `WC.GM` at initial launch; the 20 fixtures added since then (mostly Mexico/Canada-hosted, plus two additional US-hosted gaps) always run on the pure Elo model, with no live-market or what-if override available, since Polymarket's per-match slugs (`markets.js`, `ODDS_SLUGS`) haven't been extended to them yet.
